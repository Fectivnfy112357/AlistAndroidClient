...
- [ ] **Step 5**: 确认 `HomeRepository.load()` 存在（`grep "fun load" app/src/main/java/com/textvision/alistclient/home/HomeRepository.kt`）；不存在则在 HomeRepository 加最小实现 `suspend fun load(): HomeData = HomeData(...)`（用现有底层 API）
...
**Files:** Delete 11 个 `ui/components/Cloud*.kt` + `BreadcrumbBar.kt` + `DirectoryBrowser.kt` + 旧 `ui/components/TransferProgress.kt`（注意：Phase 2 创建的新 `ui/feature/transfer/TransferProgress.kt` 是不同文件，保留）+ 整个 `ui/screens/` 目录
